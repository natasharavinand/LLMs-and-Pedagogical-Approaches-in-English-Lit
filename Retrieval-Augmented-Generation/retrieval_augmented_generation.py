...
def get_llm_response_with_prompt(course, query, previous_responses_list):
    """ get_llm_response_with_prompt takes in the user's course selection, a user query, and a list of previous responses and generates a response that imitates
        ...
        previous_responses_list (List[str]): A list of previous responses the model has generated
    ...
    chromaDB_retriever = Chroma(persist_directory="./chroma", collection_name="transcripts", embedding_function=OpenAIEmbeddings(model="text-embedding-3-large")).as_retriever()
    documents = chromaDB_retriever.get_relevant_documents(query)
    context = "\n".join([doc.page_content for doc in documents])

    previous_responses = "\n".join(previous_responses_list)
    
    prompt_template = PromptTemplate(template=prompt_template, input_variables=["course", "context", "previous_responses", "query"])
    answer_chain = LLMChain(llm=llm, prompt=prompt_template, output_key="answer")

    overall_chain = SequentialChain(input_variables=["course", "context", "previous_responses", "query"],
                                    output_variables=["answer"],
                                    chains=[answer_chain], 
                                    )

    response = overall_chain.invoke({"course": course, "context":context, "previous_responses":previous_responses, "query":query})
    ...
    if response_to_user[-1] != "?":
        ...
        segue_response_template = PromptTemplate(template=segue_template, input_variables=["course", "statement", "query"])
        segue_chain = LLMChain(llm=llm, prompt=segue_response_template, output_key="answer")

        segue_response_chain = SequentialChain(input_variables=["course", "statement", "query"],
                                        output_variables=["answer"],
                                        chains=[segue_chain], 
                                        )
        
        segue = segue_response_chain.invoke({"course":course, "statement":response["answer"], "query":query})
        response_to_user += "\n\n" + segue["answer"].strip()
        
    return response_to_user

def get_user_recommendation(course, messages):
    """ get_user_recommendation takes in the user's course selection and a list of the messages in the sent (both queries posed by
    ...
    recommendation_response_template = PromptTemplate(template=recommendation_template, input_variables=["course", "messages"])
    recommendation_chain = LLMChain(llm=llm, prompt=recommendation_response_template, output_key="recommendation")

    recommendation_response_chain = SequentialChain(input_variables=["course", "messages"],
                                    output_variables=["recommendation"],
                                    chains=[recommendation_chain], 
                                    )
    
    response = recommendation_response_chain.invoke({"course":course, "messages":messages})
