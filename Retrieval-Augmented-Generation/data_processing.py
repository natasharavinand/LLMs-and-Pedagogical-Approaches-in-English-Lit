...
def extract_transcript_number(transcript_filename):
  ...
  return int(transcript_filename[transcript_filename.index(".") - 2:transcript_filename.index(".")])
...
def get_cleaned_transcript_text(transcript_text):
  ...
  transcript_text = transcript_text.replace("<< back", "").replace("[end of transcript]", "").replace("back to top", "")
...
def process_course(course_num):
  ...
  zipfile_path = f"./data/raw/engl{course_num}.zip"
  extracted_path = f"./data/extracted/engl{course_num}"
  
  if course_num == 220:
    extracted_transcripts_path = "./data/extracted/engl220/Milton/content/transcripts"
  else:
    extracted_transcripts_path = f"./data/extracted/engl{course_num}/ENGL{course_num} with 2012 Watermark/content/transcripts"
...
if __name__ == "__main__":
    ...
    for course_number in course_numbers:
        ...
        for lecture_number, transcript in result.items():
          with open(f"./data/processed/{course_number}/lecture{lecture_number}.txt", "w") as file:
              file.write(result[lecture_number])
